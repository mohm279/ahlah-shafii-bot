...
import { Send, Loader2, Home, ArrowDown } from "lucide-react";
  ...
  head: () => ({
    meta: [
      { title: "مساعد فقهي" },
      {
        name: "description",
        content:
          "مساعد ذكي يجيب على أسئلتك الفقهية وفق المذاهب الأربعة المعتمدة.",
      },
      {
        property: "og:title",
        content: "المساعد الفقهي للمذاهب الأربعة",
      },
      {
        property: "og:description",
        content: "اسأل واحصل على إجابات فقهية وفق المذاهب الأربعة.",
      },
    ],
    links: [
      {
...
function ChatPage() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col" dir="rtl">
      ...
      >
        <span className="absolute right-[2%] top-[4%] text-[180px] leading-none">
          ۞
        </span>
        <span className="absolute bottom-[4%] left-[2%] text-[180px] leading-none">
          ۞
        </span>
      </div>
      ...
      <header className="relative rounded-b-[32px] bg-gradient-hero px-6 py-10 text-white shadow-[0_20px_50px_-12px_rgba(27,200,32,0.28)]">
        ...
        <div className="relative z-20 mx-auto flex max-w-3xl items-center justify-center gap-10 text-center">
          <div>
            <h1 className="font-display text-10xl w-full px-1 py-6 font-bold tracking-wide text-gradient-gold sm:text-5xl">
              مساعد فقهي
            </h1>

            <p className="mt-1 text-xs tracking-[0.3em] text-white/80 sm:text-sm">
              إجابات وفق المذاهب الأربعة المعتمدة
            </p>
          </div>
        </div>

        {/* صورة أهلّه */}
        <div className="absolute left-6 top-1/2 z-30 flex -translate-y-1/2 items-center justify-center">
          <img
            id="home-logo"
            ...
            className="h-14 w-auto cursor-pointer rounded-xl border-2 border-[rgba(240,215,140,0.4)] object-contain shadow-gold transition-transform duration-300 hover:-rotate-5 hover:scale-105 sm:h-20"
          />
        </div>
      </header>
        ...
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <p className="max-w-md text-sm leading-relaxed text-muted-foreground">
              اسأل أي سؤال فقهي وسأجيبك وفق المذاهب الأربعة المعتمدة.
            </p>

            <div className="mt-8 grid w-full max-w-md gap-3">
              {[
                "ما حكم قراءة الفاتحة خلف الإمام في المذاهب الأربعة؟",
                "كيف تُحسب زكاة المال في المذاهب الأربعة؟",
                "ما نواقض الوضوء في المذاهب الأربعة؟",
              ].map((q) => (
                <button
                  key={q}
                  onClick={() => setInput(q)}
              ...
              <div className="flex justify-end">
                <div className="rounded-2xl border border-border bg-card px-4 py-3 shadow-sm">
                  <Loader2 className="h-4 w-4 animate-spin text-primary" />
                        جاري تجهيز الإجابة، انتظر قليلًا...
                </div>
              </div>
            )}
      ...
      <button
        ...
        className="fixed bottom-24 left-6 z-40 flex h-11 w-11 items-center justify-center rounded-full border border-border bg-card shadow-lg transition hover:-translate-y-1 hover:shadow-gold"
        title="النزول إلى آخر المحادثة"
      >
        <ArrowDown className="h-5 w-5 text-primary" />
      </button>

      {/* مكان السؤال ثابت تحت */}
      <footer className="sticky bottom-0 z-30 border-t border-border bg-card/90 backdrop-blur">
        <form
